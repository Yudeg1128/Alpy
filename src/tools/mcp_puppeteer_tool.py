...
class MCPPuppeteerTool(BaseTool, BaseModel):
    ...
    description: str = (
        ...
        "1. \"navigate\": Navigates the browser to a given URL. Can also be used to connect to an existing browser instance.\n"
        "   - Required: 'url' (string).\n"
        "   - Optional: 'launchOptions' (object). To connect to an existing browser, set `launchOptions` to `{\"browserWSEndpoint\": \"ws://127.0.0.1:9222/YOUR_ENDPOINT\"}`. Replace the port and endpoint path as needed. Otherwise, for new browser launch, can use options like `{\"headless\": false}`.\n"
        "   - Optional: 'allowDangerous' (boolean, default: false for new browser launch).\n"
        ...
        "IMPORTANT: Browser state (either newly launched or connected-to) is persistent between actions within this tool instance. Actions operate on the current page after navigation.\n\n"
        "Example - Connect to existing browser and navigate, then take a screenshot:\n"
        "Action 1:\n"
        "```json\n"
        "{\"action\": \"MCPPuppeteer\", \"action_input\": {\"action\": \"navigate\", \"url\": \"https://example.com\", \"launchOptions\": {\"browserWSEndpoint\": \"ws://127.0.0.1:9222/devtools/browser/some-uuid\"}}}\n"
        "```\n"
        "Observation 1: (Output from navigate, often just confirms success or error)\n"
        "Action 2:\n"
        "```json\n"
        "{\"action\": \"MCPPuppeteer\", \"action_input\": {\"action\": \"screenshot\", \"name\": \"example-home-existing-browser\", \"encoded\": true}}\n"
        "```\n"
        "Observation 2: (Output from screenshot, likely containing the base64 data URI if encoded=true)\n\n"
        "Example - Launch new browser, fill a login form:\n"
        "Action 1 (Navigate):\n"
        "```json\n"
        "{\"action\": \"MCPPuppeteer\", \"action_input\": {\"action\": \"navigate\", \"url\": \"https://some-login-page.com\", \"launchOptions\": {\"headless\": true}}}\n"
        "```\n"
